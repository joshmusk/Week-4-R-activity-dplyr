...
#and give it a name
hair_sex <- select(mydata,Hair,Sex)
hair_sex
...
#colnames(hair_sex)[colnames(hair_sex)=="Sex"] <- "Gender"
rename(hair_sex,Gender=Sex)

#Let's make a new dataset with the new column name
#TASK: Write the function that names a new dataset that includes the 'gender' column
newdata <- rename(hair_sex,Gender=Sex)

#Let's 'filter' just the females from our dataset
#TASK: Write the function that includes only rows that are 'female'
filter(newdata, Gender=="Female")
Femaledata <- filter(newdata, Gender=="Female")
...
#TASK: After you run it, write the total here:592____
...
filter(newdata, Gender== "Male")
Maledata <- filter(newdata, Gender== "Male")

#And now let's join the males and females
#TASK: Write the function that joins the male and female rows 
#(hint: try using 'union' or 'bind_rows')
union(Femaledata,Maledata)


#Optional Task: add any of the other functions 
#you learned about from the dplyr package
slice(mydata, 5:10)
summarise(Femaledata)
summarise(mydata,across(everything(),mean))
